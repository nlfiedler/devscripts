...
%% $ sudo ./replica.escript tank/source tank/target
%%
...
guard_replicate(FromSet, ToSet) ->
    ...
    try replicate(FromSet, ToSet) of
        ...
        Type:Error -> error_logger:error_msg("replication failed, ~p, ~p~n", [Type, Error])
    after
...
% ISO 8601 format (YYYY-mm-dd-HH:MM). The time is in UTC.
take_snapshot(DataSet) ->
    {{Year, Month, Day}, {Hour, Min, _Sec}} = erlang:universaltime(),
    Tag = io_lib:format("~4.10.0B-~2.10.0B-~2.10.0B-~2.10.0B:~2.10.0B",
...
our_snapshots(DataSet) ->
    ...
    {ok, MP} = re:compile("@replica:\\d{4}-\\d{2}-\\d{2}-\\d{2}:\\d{2}"),
    KeepOurs = fun(Elem) ->
