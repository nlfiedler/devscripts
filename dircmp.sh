...
function dodiffs {
	...
	case "$type" in
		...
		*empty*) echo $D1/`basename "$a"` is an empty file |
			  pr -h "diff of $a in $D1 and $D2" >> $TMP/dc$$g
			continue
		;;
	...
	($cmd "$D1"/"$a" "$D2"/"$a"; echo $? > $TMP/dc$$status) | \
	    pr -h "diff of $a in $D1 and $D2" >> $TMP/dc$$g
	if [[ `cat $TMP/dc$$status` != 0 ]]
...
pr -w${width} -h "$D1 only and $D2 only" -m $TMP/dc$$e $TMP/dc$$d
...
sed -e 's/..//'  -e  's/\([^-a-zA-Z0-9/_.]\)/\\\1/g' < $TMP/dc$$c > $TMP/dc$$f


cat $TMP/dc$$f | xargs ls -lLgnd | \
  sed -e '/^[bc]/ s/, *//' -e '/^l/ s/ -> .*//' > $TMP/dc$$i 2>/dev/null
cd "$D0"
cd "$D1"


cat $TMP/dc$$f | xargs ls -lLgnd | \
sed -e '/^[bc]/ s/, *//' -e '/^l/ s/ -> .*//' > $TMP/dc$$h 2>/dev/null
cd "$D0"
> $TMP/dc$$g
#
...
done 3<$TMP/dc$$h 4<$TMP/dc$$i | pr -r -h "Comparison of $D1 $D2"
if (( Dflag == 1 ))
then cat $TMP/dc$$g
fi
rm -f $TMP/dc$$*
